...
- `gpx-stabilizer` (core, `packages/core`) — bin `gpx-stabilizer` (aligned with the package name since
  2026-07-05; was `gpx-stabilize`), **zero runtime dependencies**.
...
The core must stay zero-runtime-dependency so `npx gpx-stabilizer` is light. `optionalDependencies`
...
└─ packages/
   ├─ core/    name "gpx-stabilizer"   dependencies: {}    bin gpx-stabilizer
