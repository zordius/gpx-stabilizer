...
test("stabilize: drops flagged points and reduces survivors to plain track points", () => {
  ...
  assert.ok(clean.length < pts.length); // at least the duplicate is gone
...
test("stabilizeTrack: stabilizes each segment and preserves meta", () => {
  const track = {
    segments: [
      [
        ...
        { lat: 36, lon: 138, ele: 1000, time: 0 }, // duplicate
  ...
  assert.ok(out.segments[0].length >= 2); // the duplicate was removed
});
