...
function headBoundaryTrack() {
  const jitter = [900, 900.5, 900.2, 900.9, 900.4, 901.2].map((ele, i) => pt({ ele, time: i * 1000 }));
  const sustained = Array.from({ length: 17 }, (_, i) => pt({ ele: 902 + i, time: (6 + i) * 1000, hs: 2 }));
...
test("segmentBoundaryEle: drops a jittery head prefix before sustained motion starts (no previous segment at all)", () => {
  ...
  finalize(out, { g: {} });
  for (let i = 0; i <= 5; i++) assert.deepEqual(out[i].segmentBoundaryEle, { ele: null }, `index ${i}`);
  for (let i = 6; i < out.length; i++) assert.equal(out[i].segmentBoundaryEle, undefined, `index ${i}`);
});
...
test("segmentBoundaryEle: drops a jittery tail suffix after sustained motion ends (no next segment at all)", () => {
  ...
  const sustained = Array.from({ length: 17 }, (_, i) => pt({ ele: 902 + i, time: i * 1000, hs: 2 }));
  ...
  for (let i = 17; i < out.length; i++) assert.deepEqual(out[i].segmentBoundaryEle, { ele: null }, `index ${i}`);
});

test("segmentBoundaryEle: a small gap to the neighbouring segment (not isolated) suppresses the head check", () => {
  const seg1 = Array.from({ length: 5 }, (_, i) => pt({ ele: 850, time: i * 1000, hs: 2, segment: { id: 1 } }));
...
test("segmentBoundaryEle: a gap past the threshold (not just a missing neighbour) still counts as isolated", () => {
  ...
  for (let i = 5; i <= 10; i++) assert.deepEqual(out[i].segmentBoundaryEle, { ele: null }, `seg2 jitter index ${i}`);
});
...
test("segmentBoundaryEle: cumulative climb/descent below the threshold is left alone", () => {
  const jitter = [900, 900.1, 900.05, 900.15, 900.02, 900.2].map((ele, i) => pt({ ele, time: i * 1000 })); // cumPos ~0.4m
  const sustained = Array.from({ length: 17 }, (_, i) => pt({ ele: 901 + i, time: (6 + i) * 1000, hs: 2 }));
...
test("segmentBoundaryEle: cumulative DESCENT alone (not just climb) can also trigger the drop", () => {
  const jitter = [905, 904.5, 904.8, 904.1, 904.6, 903.8].map((ele, i) => pt({ ele, time: i * 1000 })); // cumNeg ~-1.9m
  const sustained = Array.from({ length: 17 }, (_, i) => pt({ ele: 903 - i, time: (6 + i) * 1000, hs: 2 }));
  const out = [...jitter, ...sustained];
  finalize(out, { g: {} });
  for (let i = 0; i <= 5; i++) assert.deepEqual(out[i].segmentBoundaryEle, { ele: null }, `index ${i}`);
});
...
test("segmentBoundaryEle: a sustained-motion stretch shorter than the minimum duration doesn't count", () => {
  ...
  const jitter = [900, 900.5, 900.2, 900.9, 900.4, 901.2].map((ele, i) => pt({ ele, time: i * 1000 }));
  const tooShort = Array.from({ length: 10 }, (_, i) => pt({ ele: 902 + i, time: (6 + i) * 1000, hs: 2 }));
